...
### Requirement 1: startResearch 工具创建

**用户故事**: 作为前端开发者，我希望创建 startResearch 工具，以便 AI 可以启动研究任务

#### 验收标准

1. THE System SHALL 创建 startResearch 工具文件 `lib/ai/tools/start-research.ts`
2. THE startResearch 工具 SHALL 接受 topic 参数（字符串类型）
3. WHEN AI 调用 startResearch 时，THE System SHALL 向后端 API 发送 POST 请求
4. THE System SHALL 立即返回 taskId 和 status，不等待研究完成
5. THE System SHALL 返回格式: {taskId: string, status: 'started', message: string}
6. WHEN API 调用失败时，THE System SHALL 返回 {taskId: null, status: 'failed', error: string}

### Requirement 2: useResearchProgress Hook 实现

**用户故事**: 作为前端开发者，我希望有一个 Hook 订阅研究进度，以便实时显示进度

#### 验收标准

1. THE System SHALL 创建 useResearchProgress Hook 文件 `hooks/use-research-progress.ts`
2. THE Hook SHALL 接受 taskId 参数（可选）
3. WHEN taskId 存在时，THE Hook SHALL 创建 EventSource 连接
4. THE Hook SHALL 监听 SSE 事件: start, plan, progress, done, error
5. THE Hook SHALL 维护状态: events, status, report, error
6. THE Hook SHALL 实现自动重连机制（最多 3 次）
7. THE Hook SHALL 使用指数退避策略（500ms, 1s, 2s）
8. WHEN 组件卸载时，THE Hook SHALL 关闭 EventSource 连接
...
2. THE System SHALL 支持 POST 方法（启动新研究）
3. THE System SHALL 支持 GET 方法（用于 EventSource）
4. THE System SHALL 从环境变量读取 RESEARCH_API_URL
5. THE System SHALL 代理请求到 FastAPI 后端
6. THE System SHALL 返回 SSE 流（text/event-stream）
7. THE System SHALL 设置正确的响应头（Cache-Control, X-Accel-Buffering）
...
2. THE Component SHALL 接受 taskId 属性
3. WHEN taskId 为 null 时，THE Component SHALL 不显示任何内容
4. THE Component SHALL 显示当前状态图标（运行中、完成、失败）
5. THE Component SHALL 显示进度列表（每个步骤）
6. THE Component SHALL 显示错误信息（如果失败）
7. THE Component SHALL 使用 useResearchProgress Hook
8. THE Component SHALL 有清晰的视觉设计（图标、颜色、动画）
...
1. THE System SHALL 在聊天 API 路由中集成 startResearch 工具
2. THE System SHALL 更新 System Prompt 指导 AI 使用研究工具
3. WHEN 用户请求研究时，THE AI SHALL 调用 startResearch 工具
4. WHEN startResearch 返回 taskId 时，THE AI SHALL 告知用户研究已开始
5. THE System SHALL 在聊天界面显示 ResearchProgress 组件
6. WHEN 研究完成时，THE AI SHALL 调用 createDocument 创建 Artifact
7. THE System SHALL 在 Artifact 中显示研究报告

### Requirement 6: System Prompt 更新

**用户故事**: 作为开发者，我希望 System Prompt 能够指导 AI 正确使用研究工具

#### 验收标准

1. THE System SHALL 更新 System Prompt 文件 `lib/ai/prompts.ts`
2. THE System Prompt SHALL 明确说明何时使用 startResearch 工具
3. THE System Prompt SHALL 说明工具调用顺序: startResearch → createDocument
