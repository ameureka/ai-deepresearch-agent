...
**1. 统一 AI 架构**
- ✅ 前端和后端统一使用 DeepSeek
- ✅ 移除 Vercel AI Gateway 依赖
- ✅ 降低成本 80%+
- ✅ 简化配置（只需一个 DEEPSEEK_API_KEY）
...
**问题**：
- ❌ 双 AI 系统（Grok + DeepSeek）
- ❌ 成本高（Grok 比 DeepSeek 贵 10 倍+）
- ❌ 配置复杂（需要 AI Gateway + DeepSeek）
- ❌ 单点故障（前端 AI 失败 = 整个功能不可用）
- ❌ 流程冗长（9 个步骤）
- ❌ 关键词检测不可靠
...
└─────────────────┴─────────────────┘
   ↓
后端研究 (DeepSeek Reasoner) - 成本：$
   ↓
研究完成
   ↓
直接展示结果（Artifact 或内联显示）
```

**优势**：
- ✅ 统一 AI 系统（DeepSeek）
- ✅ 成本降低 80%+
- ✅ 配置简单（只需 DEEPSEEK_API_KEY）
- ✅ 容错性强（双触发模式）
- ✅ 流程简化（4 个步骤）
- ✅ 用户体验更好
...
| 项目 | 原设计 (v1.0) | 新设计 (v2.0) | 节省 |
|------|--------------|--------------|------|
| 前端聊天 | Grok: $15/M tokens | DeepSeek: $0.14/M tokens | 99% |
| 后端研究 | DeepSeek: $0.14/M tokens | DeepSeek: $0.14/M tokens | 0% |
| 总成本 | ~$15.14/M tokens | ~$0.28/M tokens | 98% |

**实际使用场景**：
- 每天 100 次对话，每次 1000 tokens
- 每天 10 次研究，每次 10000 tokens

```
原设计成本：
- 前端：100 × 1000 × $15/1M = $1.50/天
- 后端：10 × 10000 × $0.14/1M = $0.014/天
- 总计：$1.514/天 × 30 = $45.42/月

新设计成本：
- 前端：100 × 1000 × $0.14/1M = $0.014/天
- 后端：10 × 10000 × $0.14/1M = $0.014/天
- 总计：$0.028/天 × 30 = $0.84/月
...
# 或
VERCEL_OIDC_TOKEN=xxx   # Vercel 部署时自动生成

# 后端 (.env)
DEEPSEEK_API_KEY=xxx
OPENAI_API_KEY=xxx      # 降级备用
```
...
# 后端 (.env)
DEEPSEEK_API_KEY=xxx    # 同一个 key
OPENAI_API_KEY=xxx      # 降级备用（可选）
```

**优势**：
- ✅ 只需一个 DeepSeek API Key
- ✅ 无需 Vercel 账号
- ✅ 配置简单
...
│  │  │ 📋 Copy to Clipboard                   │                   │
│  │  │ 💾 Export as Markdown                  │                   │
│  │  │ 🔄 Share with AI for further analysis  │ ⭐ 可选           │
│  │  │ 🔍 Start new research                  │                   │
│  │  └────────────────────────────────────────┘                   │
│  │                                                                │
│  ├─> T10: 用户选择"Share with AI" (可选)                         │
│  │                                                                │
│  │  [发送给 AI 进一步分析]                                        │
│  │  - 用户主动选择，而非自动发送                                  │
│  │  - AI 可以基于报告回答问题                                     │
